...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        // 1. Receive TTS audio from bus, forward to Swift worker for playback
        while let Some(sample) = spk_sub.receive()? {
            let payload = sample.payload();
            if payload.len() < 4 { continue; }

            // payload is f32 PCM at 24kHz from tts-ant
            let sample_count = (payload.len() / 4) as i32;

            // Write to Swift worker stdin: [i32 count][f32 samples]
            ...
            // Boost audio 4x to counter Voice Processing AGC ducking
        ...
        // 2. Receive echo-cancelled mic audio from Swift worker, publish to stt_raw
