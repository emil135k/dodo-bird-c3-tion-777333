...
struct PatchbayConfig {
    ...
    #[serde(default = "d_aec_delay")]
    aec_delay_ms: u32,
}
fn d_aec_delay() -> u32 { 80 }
...
impl Default for PatchbayConfig {
    fn default() -> Self { Self { input_device: d_input(), output_device: d_output(), aec_delay_ms: d_aec_delay() } }
}
impl PatchbayConfig {
    fn load() -> Self {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let out_rate_f64 = out_rate as f64;
    ...
    let output_stream = output_dev.build_output_stream(
        ...
        move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
            ...
            // Collect samples at output rate for AEC reference
            let mut played_at_out_rate: Vec<f32> = Vec::new();
            for frame in data.chunks_mut(out_channels) {
                let sample = pq.pop_front().unwrap_or(0.0);
                for ch in frame.iter_mut() {
                    *ch = sample;
                }
                played_at_out_rate.push(sample);
            }

            // Vale's fix: resample directly from out_rate → 16kHz via linear interpolation
            // No 48kHz intermediate. Clean single-step resample.
            if !played_at_out_rate.is_empty() && played_at_out_rate.iter().any(|&s| s.abs() > 0.001) {
                let ratio = out_rate_f64 / 16000.0; // e.g. 44100/16000 = 2.75625
                let out_len = (played_at_out_rate.len() as f64 / ratio) as usize;
                let mut ref_16k = Vec::with_capacity(out_len);
                for i in 0..out_len {
                    let src = i as f64 * ratio;
                    let idx = src as usize;
                    let frac = (src - idx as f64) as f32;
                    let s0 = played_at_out_rate[idx.min(played_at_out_rate.len() - 1)];
                    let s1 = played_at_out_rate[(idx + 1).min(played_at_out_rate.len() - 1)];
                    let s = s0 + (s1 - s0) * frac;
                    ref_16k.push((s * 32767.0).clamp(-32768.0, 32767.0) as i16);
                }
    ...
    // Vale's fix: configurable delay line for reference alignment
    let ref_delay_samples = (cfg.aec_delay_ms as usize * AEC_RATE as usize) / 1000;

    eprintln!("[PATCHBAY] SpeexDSP AEC: {}Hz, frame={}, filter={}ms, ref_delay={}ms",
        AEC_RATE, AEC_FRAME_SIZE, AEC_FILTER_LENGTH as f64 / AEC_RATE as f64 * 1000.0,
        cfg.aec_delay_ms);
    eprintln!("[PATCHBAY] AEC ref captured at hardware playback time — no rodio");
        ...
        // 2. Process mic capture through AEC
        {
            ...
            if buf.len() >= needed {
                ...
                while buf.len() >= needed {
                    ...
                    // Vale's fix: delay line — only pull reference after delay buffer fills
                    let echo_16k: Vec<i16> = {
                        let mut sr = speaker_ref.lock().unwrap();
                        if sr.len() >= ref_delay_samples + AEC_FRAME_SIZE {
