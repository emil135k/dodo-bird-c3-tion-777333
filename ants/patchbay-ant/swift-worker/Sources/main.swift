...
struct PatchbayWorker {
    static func main() async {
        ...
        while true {
            ...
            // Negative values = control commands from speaker-ctl-ant via patchbay
            // -1 = FLUSH (stop + play), -2 = PAUSE, -3 = RESUME
            if sampleCount < 0 {
                switch sampleCount {
                case -1:
                    playerNode.stop()
                    playerNode.play()
                    fputs("[PATCHBAY-WORKER] CTL: FLUSH — cleared queue, ready for next\n", stderr)
                case -2:
                    playerNode.pause()
                    fputs("[PATCHBAY-WORKER] CTL: PAUSE — speaker silenced, mic alive\n", stderr)
                case -3:
                    playerNode.play()
                    fputs("[PATCHBAY-WORKER] CTL: RESUME — speaker active\n", stderr)
                default:
                    fputs("[PATCHBAY-WORKER] CTL: unknown command \(sampleCount)\n", stderr)
                }
                continue
            }
            if sampleCount == 0 { continue }
            if sampleCount > 4800000 {
